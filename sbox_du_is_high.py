...
def build_dataset(n=2000):
    data = []
    for _ in range(n):
        data.append(torch.tensor(affine_transform(AES_SBOX), dtype=torch.float32))
...
def bijection_loss(sbox):
    s_int = torch.clamp(torch.round(sbox), 0, 255).to(torch.int64)
    unique = len(torch.unique(s_int))
    return 256 - unique

def differential_uniformity_loss(sbox):
    # Ensure valid S-box values 0..255
    ...
    # Precompute S(x) and S(x⊕a) for all a
    x = torch.arange(256, device=device).unsqueeze(0)          # shape: (1, 256)
    a = torch.arange(1, 256, device=device).unsqueeze(1)       # shape: (255, 1)

    # Compute x⊕a for all pairs
    xa = x ^ a                                                 # shape: (255, 256)

    # Lookup S(x) and S(x⊕a)
    s_x  = s[x]                                                # shape: (1, 256)
    s_xa = s[xa]                                               # shape: (255, 256)

    # Compute output differences Δy = S(x) ⊕ S(x⊕a)
    diffs = s_x ^ s_xa                                         # shape: (255, 256)

    # Flatten for bincount
    diffs_flat = diffs.reshape(-1)

    # Count occurrences of each Δy (0..255)
    counts = torch.bincount(diffs_flat, minlength=256)

    # But we need DU per each a, not global — so compute per-row max
    # Build histogram per row using one-hot encoding
    onehot = torch.nn.functional.one_hot(diffs, num_classes=256).sum(dim=1)

    # DU(a) = max count in each row
    du_per_a = onehot.max(dim=1).values

    # Return average DU
    return du_per_a.float().mean()

def nonlinearity_loss(sbox):
    s = torch.clamp(torch.round(sbox), 0, 255).to(torch.float32).to(device)
    
    try:
        # GPU-optimized: pre-compute all linear basis vectors at once
        linear_basis = torch.zeros((256, 256), dtype=torch.float32, device=device)
        for a in range(256):
            linear_basis[a] = torch.tensor(
                [(a & x).bit_count() % 2 for x in range(256)],
                dtype=torch.float32,
                device=device
            )
        
        # Vectorized dot products: s @ linear_basis^T
        correlations = torch.abs(torch.matmul(s, linear_basis.t()))
        ...
        for a in range(256):
            ...
            corr = torch.abs(torch.dot(s, linear))
            nl += corr
        return nl / 256
...
def train(resume=True):
    ...
    for epoch in pbar:
        ...
        loss_G = -D(fake).mean()
        loss_G += 0.1 * bijection_loss(fake[0])
        loss_G += 0.01 * differential_uniformity_loss(fake[0])
        loss_G += 0.01 * nonlinearity_loss(fake[0])
        ...
        pbar.set_postfix({"G_Loss": f"{loss_G.item():.2f}", "D_Loss": f"{loss_D.item():.2f}"})
...
def generate_sbox():
    ...
    generated_sbox = generated_sbox.argsort()
