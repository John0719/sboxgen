...
def differential_uniformity_loss(sbox):
    ...
    for a in range(1, 256):
        ...
        for x in range(256):
            ...
            v2 = s_cpu[(x ^ a) & 0xFF]
            diffs.append(v1 ^ v2)
        diffs_tensor = torch.tensor(diffs, dtype=torch.int64)
        counts = torch.bincount(diffs_tensor, minlength=256)
        du += counts.max()
    return du / 256

def nonlinearity_loss(sbox):
    s = torch.clamp(torch.round(sbox), 0, 255).to(torch.float32).cpu()
    nl = 0.0
    for a in range(256):
        linear = torch.tensor(
